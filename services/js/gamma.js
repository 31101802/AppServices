...
var Gamma = (function () {
	...
	var $window = $(window),
		...
		_saveState = function (id) {
			var guid = History.getState().url.queryStringToJSON().guid;
			if (!Gamma.settings.historyapi && id != undefined) {
				...
				History.pushState(null, null, url('protocol') + '://' + url('hostname') + url('path') + '?guid=' + guid);
			...
			// this will trigger the statechange on the window
				...
				History.pushState(null, null, '?guid=' + guid + '&id=' + id);
	...
	_singleviewitem = function ($item, anim) {
		...
		setTimeout(function () {
			...
			_applyAnimation(Gamma.overlay, {
				'opacity': 1
			}, Gamma.settings.speed, Gamma.supportTransitions || !anim, function () {
			...
			if (Gamma.fly) {
				...
				setTimeout(function () {
					...
					_applyAnimation(Gamma.fly, {
						'opacity': 0
					}, 1000, Gamma.supportTransitions, function () {
	...
	_prepareSlideshow = function () {
		...
		_toggleControl(Gamma.svnavprev, 'off', {
			left: -40
		});
		_toggleControl(Gamma.svnavnext, 'off', {
			right: -40
		});
	...
	_stopSlideshow = function (pause) {
		...
		if (!pause) {
			...
			_toggleControl(Gamma.svnavprev, 'on', {
				left: 20
			});
			_toggleControl(Gamma.svnavnext, 'on', {
				right: 20
			});
			_svResizeImage(function () {
